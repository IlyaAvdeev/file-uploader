...
class Uploader(@ConfigProperty(name = "storage.path") val storagePath: String, @Inject val log:Logger) {
    ...
    fun uploadFile(@RestForm("file") file: FileUpload?):Response {
        val resultPath = Files.copy(file!!.filePath(), Paths.get(storagePath, file.fileName()), StandardCopyOption.REPLACE_EXISTING)
